...
document.addEventListener("DOMContentLoaded", () => {
    ...
    let currentCount = parseInt(localStorage.getItem("cartCount") || "0", 10);
    if (cartBadge) cartBadge.textContent = currentCount;

    const params = new URLSearchParams(window.location.search);
    const id = params.get("id");

    if (id === null || id === undefined || !products[id]) {
        document.querySelector(".detail-workspace-root").innerHTML = `
            <div style="text-align: center; padding: 60px 20px;">
                <h2 style="color: #b12704; margin-bottom: 10px;">Product Record Missing</h2>
                <p>The system could not fetch the specified database index lookup reference.</p>
                <a href="index.html" style="color: #007185; text-decoration: none; display: inline-block; margin-top: 15px;">← Back to home shopping wall</a>
            </div>
        `;
        return;
    }
    ...
    if (prodDescription) prodDescription.textContent = product.description;

    document.title = `${product.name} - Amazon.in Buy Details`;

    const addBtn = document.querySelector(".checkout-add");
    if (addBtn) {
